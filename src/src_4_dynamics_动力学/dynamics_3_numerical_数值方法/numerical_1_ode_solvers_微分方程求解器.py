...
class RK4Solver:
    ...
    def step(self):
        k1 = self.dt * self.ode_func(self.t, self.state)
        k2 = self.dt * self.ode_func(self.t + self.dt / 2, self.state + k1 / 2)
        k3 = self.dt * self.ode_func(self.t + self.dt / 2, self.state + k2 / 2)
        k4 = self.dt * self.ode_func(self.t + self.dt, self.state + k3)
